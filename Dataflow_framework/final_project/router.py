...
from typing import Dict, Any, Tuple, Iterator, List, Deque, Optional
...
class RoutingEngine:
    ...
    and file processing lifecycle (in watch mode).
    """
    def __init__(self, config_path: str, watch_dir: Optional[str] = None, enable_tracing: bool = False):
        """
        Initializes the RoutingEngine by loading configuration and processors,
        setting up metrics, tracing, and file monitoring paths (if in watch mode).

        Args:
            config_path: Path to the configuration file (e.g., 'config/config.yaml').
            watch_dir: The base directory to monitor for files (required in watch mode).
            enable_tracing: Boolean flag to enable/disable line tracing.
        """
        self.config = self._load_config(config_path)
        self.processors = self._load_processors(self.config)

        self.is_watch_mode = watch_dir is not None

        # File monitoring directories (only relevant in watch mode)
        self.watch_dir = watch_dir
        self.unprocessed_dir = os.path.join(watch_dir, 'unprocessed') if self.is_watch_mode else None
        self.underprocess_dir = os.path.join(watch_dir, 'underprocess') if self.is_watch_mode else None
        self.processed_dir = os.path.join(watch_dir, 'processed') if self.is_watch_mode else None


        self.routing_queue: Deque[ProcessingItem] = deque()
        # Track lines per file to know when a file is fully processed (only in watch mode)
        ...
        # File state metrics (only relevant in watch mode)
    ...
    def add_line(self, tag: str, line: str, file_path: str):
        ...
        # Track the line count for this file only in watch mode
    ...
    def process_queue_continuously(self):
        """Processes items from the routing queue in a continuous loop (for watch mode)."""
        if not self.is_watch_mode:
             logging.error("process_queue_continuously called in non-watch mode. This method is for watch mode only.")
             return
        ...
        while True: # Run continuously
            ...
            for _ in range(items_to_process):
                ...
                # Update current processing file state for dashboard (only in watch mode)
                if self.is_watch_mode:
                    with self._file_state_lock:
                         self._current_processing_file = os.path.basename(file_path)
                ...
                if current_tag == 'end':
                    ...
                    # Decrement line count for the file and check if processing is complete (only in watch mode)
                    if self.is_watch_mode:
                ...
                if current_tag not in self.processors:
                    ...
                    # Decrement line count for the file as this line is discarded (only in watch mode)
                    if self.is_watch_mode:
                        self._decrement_line_count(file_path, line_to_process)
                    continue
                ...
                try:
                    ...
                    if processor_output_iterator is None:
                        ...
                        # This line is effectively discarded, so decrement line count for the file (only in watch mode)
                        if self.is_watch_mode:
                            self._decrement_line_count(file_path, line_to_process)
                    ...
                    if not output_items:
                         ...
                         # This line is discarded, so decrement line count for the file (only in watch mode)
                         if self.is_watch_mode:
                            self._decrement_line_count(file_path, line_to_process)
                    ...
                    for next_tag, processed_line, updated_trace, emitted_file_path in output_items:
                        ...
                        if next_tag not in self.processors and next_tag != 'end':
                            ...
                            # This emitted line is discarded, so decrement line count for the file (only in watch mode)
                            if self.is_watch_mode:
                    ...
                    with self._metrics_lock:
                         self.metrics[current_tag]['error_count'] += 1
                    # This line caused an error and is effectively discarded, so decrement line count for the file (only in watch mode)
                    if self.is_watch_mode:
                        self._decrement_line_count(file_path, line_to_process)
                finally:
                     end_time = time.time()
            ...
            # After processing a batch of items, update the current file state if the queue is now empty (only in watch mode)
            if self.is_watch_mode and not self.routing_queue:
                 with self._file_state_lock:
                      self._current_processing_file = None # No file is actively being processed right now


    def process_queue(self):
        """Processes items from the routing queue until it's empty (for single file mode)."""
        if self.is_watch_mode:
             logging.error("process_queue called in watch mode. This method is for single file mode only.")
             return

        logging.info("Starting single file queue processing.")
        processed_count = 0
        ...
        while self.routing_queue and current_steps < max_processing_steps:
            ...
            try:
                ...
                # --- Validation check ---
                if queue_item is None or not isinstance(queue_item, tuple) or len(queue_item) != 4:
                    logging.error(f"Queue returned invalid item unexpectedly at step {current_steps}: {queue_item}. Skipping.")
                    with self._error_lock:
                         self.error_history.append(('router', f"Invalid item in queue: {queue_item}", "N/A", "N/A"))
                    continue
                ...
                current_tag, line_to_process, trace, file_path = queue_item
                processed_count += 1
                logging.debug(f"Processing item {processed_count} (Step {current_steps}): Tag='{current_tag}', Line='{line_to_process}' from file '{file_path}'")
            ...
            if current_tag == 'end':
                ...
                # In single file mode, we don't track lines per file for completion
                # and don't remove from active_lines as the program exits anyway.
                continue # This line is done, move to the next in the queue
        ...
        # In single file mode, we don't track active lines for file completion.


    def _decrement_line_count(self, file_path: str, line_content: str):
        """Decrements the active line count for a file and moves the file if count reaches zero (only in watch mode)."""
        if not self.is_watch_mode:
             logging.warning("_decrement_line_count called in non-watch mode. Skipping.")
             return
                ...
                if self._lines_per_file[file_path] <= 0:
                    ...
                    try:
                        ...
                        with self._file_state_lock:
                         ...
                         # Assume it was moved or handled, decrementing underprocess count anyway
                         with self._file_state_lock:
                              self._file_counts['underprocess'] -= 1
                    except Exception as e:
        ...
        # Remove the line from active tracking (less critical with file tracking)
        with self._active_lines_lock:
    ...
    def get_file_state(self) -> Dict[str, Any]:
        """Returns a snapshot of the current file processing state (only relevant in watch mode)."""
        if not self.is_watch_mode:
             return {"message": "File state tracking is only available in watch mode."}
